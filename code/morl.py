...
@numba.jit(numba.boolean(numba.types.Array(numba.float64, 1, "C"), numba.types.Array(numba.float64, 1, "C")), nopython=True, fastmath=True, cache=True)
def is_dominated_by(x1: NDArray, x2: NDArray) -> bool:
    assert len(x1) == len(x2)
    # bigger is better
    if np.allclose(x1, x2, rtol=1e-5, atol=1e-8, equal_nan=False):
        return False
    else:
        out = True
...
@numba.jit(numba.types.Array(numba.boolean, 1, "C")(numba.types.Array(numba.float64, 2, "C")), nopython=True, fastmath=True, cache=True)
def check_pf(Rs: NDArray) -> NDArray:
    
    assert len(np.shape(Rs)) == 2
    _Z = Rs * 0
    for k in range(len(Rs[0])):
        _Z[:, k] = np.argsort(np.argsort(Rs[:, k]))
    Z_ = np.array([np.mean(u) for u in _Z])
    Z = np.argsort(Z_)[::-1]
    ll = int(len(Z) * 0.1)
    Z[:ll] = np.random.permutation(Z[:ll])
    
    candidates = set(Z)
    
    result = np.array([False] * len(Z))
    for num, i in enumerate(Z):
        R = Rs[i]
        out = True
        dame = set()
        for j in candidates - set([i]):
            R_o = Rs[j]
            if is_dominated_by(R_o, R):
                dame.add(j)
            elif is_dominated_by(R, R_o):
                out = False
                dame.add(i)
                break
        result[num] = out
    ...
    return result[np.argsort(Z)]


def unique_PF_points(Rs: NDArray):
    _RR = np.array(Rs).astype(np.float64)
    _RR = _RR[check_pf(_RR)]
...
def deviation_from_pf(Rs: ArrayLike):
    Rs = np.atleast_2d(Rs).astype(np.float64)
...
def run_episode_REPEAT(policy: PolicyNet, env: gym.Env, c: float, 
                       deterministic: bool, n_obj: int, gamma: float, 
                       n_episode_repeat: int, return_T: bool = False):
    Rs = []
    env = deepcopy(env)
    Ts = []
    ...
    if return_T:
        return np.mean(Rs, axis=0), Ts if n_episode_repeat > 1 else Ts[0]
    else:
...
def group_evaluation_REPEAT(policy: PolicyNet, group_size: int, n_jobs: int, env: gym.Env, 
                            deterministic: bool, n_obj: int, gamma: float,
                            n_episode_repeat: int, return_T: bool = False, seed: Optional[int] = None):
    if seed is None:
        cs = np.random.uniform(size=(group_size, policy.c_dim))
    else:
        cs = np.random.RandomState(seed).uniform(size=(group_size, policy.c_dim))
    ...
    record = Parallel(n_jobs=n_jobs)(delayed(run_episode_REPEAT)\
                                     (*(c_policy, env, c, deterministic, n_obj, gamma,
                                        n_episode_repeat, return_T)) for c in cs)
    return cs, record
...
class FE_layer(nn.Module):
    ...
    def __init__(self, embed_dims: Optional[ArrayLike]):
        super().__init__()
        self.embed_dims = embed_dims
...
class ValueNet(nn.Module):
    def __init__(self, input_dim: int, hidden_size: int, 
                 output_dim: int, lr: float, 
                 embed_dims: Optional[ArrayLike], std: float = 0.2):
        super().__init__()
        if embed_dims is None:
            IN_DIM = input_dim
        else:
...
def learn(policy: PolicyNet, gradient_steps: int, n_grad_repeat: int, 
          ...
          test_run_is_deterministic: bool = True, 
          print_unique_pf_points: bool = False, 
          lr_pi: float = 1e-3, 
    ...
    history = []    # history of metric for a deterministic policy
    if test_run_is_deterministic == False:
        history_stoc = []    # history of metric for a stochastic policy
    ...
    if use_QV:
        if act_space[0] == 'continuous':
            ...
            VV = ValueNet(input_dim=env.observation_space.shape[0], output_dim=1, 
                          ...
                          embed_dims=policy.embed_dims, lr=B_lr)
    ...
    if test_run_is_deterministic:
        history += [current_HV]
        print('{} [{:.4}]'.format(0, history[0]), end=' ')
    else:
        history += [(seed, current_HV)]
        
        Rs = group_evaluation_REPEAT(policy=policy, 
                                     group_size=group_size_test,
                                     n_jobs=n_jobs, seed=seed, 
                                     env=env.set_ep_len(ep_len_test),
                                     deterministic=False, n_obj=n_obj, gamma=gamma_test, 
                                     n_episode_repeat=test_repeat)[1]

        Rs_normalized = [(R - J_AU) / (J_U - J_AU) for R in Rs]
        current_HV = HV(Rs=Rs_normalized, ref_point=np.zeros(n_obj))
        history_stoc += [(seed, current_HV)]
        print('{} [{:.4} | {:.4}]'.format(0, history[0][1], history_stoc[0][1]), end=' ')
    # --------------------------------------------------------------------------------------------------------------------
        ...
        if test_run_is_deterministic:
            ...
            if log_file is not None:
                with open(log_file, 'a') as f:
                    f.write('{} [{:.4}]    '.format(gradient_steps - remaining_steps, history[-1]))
        else:
            history += [(seed, current_HV)]
            ...
            current_HV = HV(Rs=Rs_normalized, ref_point=np.zeros(n_obj))
            history_stoc += [(seed, current_HV)]
            print('{} [{:.4} | {:.4}]'.format(gradient_steps - remaining_steps, 
                                      history[-1][1], history_stoc[-1][1]), end=' ')
            if log_file is not None:
                with open(log_file, 'a') as f:
                    f.write('{} [{:.4} | {:.4}]    '.format(gradient_steps - remaining_steps, 
                                      history[-1][1], history_stoc[-1][1]))

    if test_run_is_deterministic:
        return history, policy, best_policy
    else:
        return (history, history_stoc), policy, best_policy
